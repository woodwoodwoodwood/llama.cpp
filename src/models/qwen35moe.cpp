...
llama_model_qwen35moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    if (params.cparams.mtp_chain && n_tokens > 1 && ubatch.n_seqs_unq == 1 && ubatch.token) {
        ...
        auto build_block = [&](ggml_tensor * cur_in, int64_t row0, int64_t width) -> ggml_tensor * {
            ...
            // MoE FFN — routed experts plus gated shared expert (mirrors qwen35moe trunk)
            ggml_tensor * moe_out_b =
                build_moe_ffn(cur_b,
                    layer.ffn_gate_inp,
                    layer.ffn_up_exps,
                    layer.ffn_gate_exps,
            ...
            if (layer.ffn_up_shexp != nullptr) {
                ...
                ggml_tensor * shared_gate_b = build_lora_mm(layer.ffn_gate_inp_shexp, cur_b);
                shared_gate_b = ggml_sigmoid(ctx0, shared_gate_b);

                ffn_shexp_b = ggml_mul(ctx0, ffn_shexp_b, shared_gate_b);

                cur_b = ggml_add(ctx0, moe_out_b, ffn_shexp_b);
            } else {
                cur_b = moe_out_b;
            }
