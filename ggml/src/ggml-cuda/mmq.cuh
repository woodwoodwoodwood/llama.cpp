...
static constexpr __host__ __device__ tile_x_sizes mmq_get_dp4a_tile_x_sizes(ggml_type type, int mmq_y) {
    switch (type) {
        ...
        case GGML_TYPE_GSQ2:    return MMQ_DP4A_TXS_Q8_0;
...
static constexpr __host__ __device__ int mmq_get_mma_tile_x_k(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_GSQ2:    return MMQ_MMA_TILE_X_K_Q8_0;
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_gsq2(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_get_nwarps_device();
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
    ...
    // GSQ2 shares the Q8_0 tile layout: unpack the 2-bit codes into signed int8
    // (value = code - 2) and replicate the single per-128-group f16 scale across
    // the 4 Q8_1 (32-element) scale slots, exactly like Q1_0 but 2 bits/code.
    constexpr int blocks_per_iter = MMQ_ITER_K / QK_GSQ2;
    constexpr int threads_per_row = blocks_per_iter * QI_GSQ2;
    constexpr int nrows = warp_size / threads_per_row;
    constexpr int ints_per_thread = QK_GSQ2 / (QI_GSQ2 * 4); // 4 output int32 per thread
    constexpr int scale_entries_per_block = QK_GSQ2 / QK8_1;
    constexpr int scale_entries_per_row = blocks_per_iter * scale_entries_per_block;

    const int txi  = threadIdx.x % threads_per_row;
    const int kbx  = txi / QI_GSQ2;
    const int kqsx = txi % QI_GSQ2;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const block_gsq2 * bxi = (const block_gsq2 *) x + kbx0 + i*stride + kbx;
        const int qs_offset = 4*kqsx;
        const int qs0 = bxi->qs[qs_offset + 0] | (bxi->qs[qs_offset + 1] << 8) |
                        (bxi->qs[qs_offset + 2] << 16) | (bxi->qs[qs_offset + 3] << 24);

        // qs0 holds 16 x 2-bit codes (LSB-first) -> 4 ints of 4 signed int8 each.
        int unpacked_bytes[ints_per_thread];
#pragma unroll
        for (int j = 0; j < ints_per_thread; ++j) {
            const int shift = j * 8;
            const int s0 = ((qs0 >> (shift + 0)) & 0x3) - 2;
            const int s1 = ((qs0 >> (shift + 2)) & 0x3) - 2;
            const int s2 = ((qs0 >> (shift + 4)) & 0x3) - 2;
            const int s3 = ((qs0 >> (shift + 6)) & 0x3) - 2;
            unpacked_bytes[j] = (s0 & 0xFF) | ((s1 & 0xFF) << 8) | ((s2 & 0xFF) << 16) | ((s3 & 0xFF) << 24);
        }

        const int dst_offset = kbx*(scale_entries_per_block*QI8_0) + kqsx*ints_per_thread;
#pragma unroll
        for (int j = 0; j < ints_per_thread; ++j) {
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + dst_offset + j] = unpacked_bytes[j];
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
        ...
        const block_gsq2 * bxi = (const block_gsq2 *) x + kbx0 + i*stride + scale_block;
...
template <int mmq_x, int mmq_y, bool need_check>
struct mmq_type_traits<mmq_x, mmq_y, need_check, GGML_TYPE_GSQ2> {
    static constexpr int              vdr          = VDR_GSQ2_Q8_1_MMQ;
    static constexpr load_tiles_mmq_t load_tiles   = load_tiles_gsq2<mmq_y, need_check>;
    static constexpr vec_dot_mmq_t    vec_dot_mma  = vec_dot_q8_0_q8_1_mma<mmq_x, mmq_y, MMQ_Q8_1_DS_LAYOUT_D4>;
    static constexpr vec_dot_mmq_t    vec_dot_dp4a = vec_dot_q8_0_q8_1_dp4a<mmq_x, mmq_y>;
};
...
extern DECL_MMQ_CASE(GGML_TYPE_GSQ2);
